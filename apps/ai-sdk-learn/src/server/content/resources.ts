...
/**
 * Fetches content modules (type: 'module') filtered by SITE_NAME
 */
export async function getModules(): Promise<ContentResource[]> {
	...
	if (!siteName) {
		console.warn(
			'getModules: process.env.SITE_NAME is not set. No site filtering applied to modules.',
		)
		// Optionally return empty array or throw error if siteName is required
		// return [];
	}
	...
	try {
		const result = await db
			...
			.where(
				and(
					eq(contentResource.type, 'module'),
					// Add condition to check if fields.sites contains siteName
					siteName
						? sql`${contentResource.fields}->'sites' @> ${JSON.stringify([siteName])}`
						: undefined, // Only apply filter if siteName is set
				),
			)
