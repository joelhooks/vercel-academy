...
async function migrateContent(isDryRun = false) {
	...
	try {
		...
		const ops = {
			toInsert: [] as ContentResource[],
			toUpdate: [] as { dbId: string; updates: Partial<ContentResource> }[], // Store dbId and changes
		...
		async function diffAndPlanOperations(
			...
			if (!existingNodeData) {
				// Insert new resource
				// Use the temporarily generated ID for now, it will be replaced post-insert
				ops.toInsert.push(generatedNode.resource)
				// We need to link this originalId to the *actual* dbId after insert
				...
				const existingFields = existingNodeData.resource.fields
				const updates: Partial<ContentResource['fields']> = {}
				...
				if (generatedFields?.summary !== existingFields?.summary) {
					// Compare summary too
				...
				// Mark for relationship rebuild in case children changed
			...
			await db.transaction(async (tx) => {
				...
				if (ops.toUpdate.length > 0) {
					...
					for (const updateOp of ops.toUpdate) {
						...
						// Ensure fields are treated as objects, handling potential null/stringified JSON
						let existingFields: Record<string, any> = {} // Use Record<string, any> for flexibility
						if (currentResource?.fields) {
							if (typeof currentResource.fields === 'string') {
								...
								existingFields = currentResource.fields as Record<string, any>
							}
						}
						...
						const partialUpdates =
							...
							updateOp.updates.fields !== null
								? updateOp.updates.fields
								: {}

						// Merge existing fields with the partial updates
						const newFields = { ...existingFields, ...partialUpdates }

						// Set the entire merged fields object
						await tx
							.update(contentResource)
							.set({ fields: newFields }) // Set the fully merged object
							.where(eq(contentResource.id, updateOp.dbId))
						bar.increment()
					}
				}
...
main()
	...
	.catch((error) => {
		...
		// Error is already logged in the main catch block
	...
	.finally(async () => {
		...
		// Force exit after cleanup attempt, regardless of success/failure
		if (process.exitCode === 1) {
			// Already exiting due to error
		} else {
			process.exit(0) // Force exit on success
		}
	})
